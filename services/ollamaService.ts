...
平衡规则（尤其是健康属性）：
- 非重大事件的健康变化范围为[-5, +12]，避免过度扣减
- 只有在明确的事故/重病下，健康可-6~-20，需给出清晰原因
- 当健康<30时，优先给出休养/康复/改善习惯的事件，避免持续扣减
- 仅当健康<5且发生明确致命事件或高龄衰老时标记死亡

输出要求：
- 只输出JSON格式，不要任何额外文字
- content字段：50-120字，生动具体，有细节描写
- statChanges：合理范围（-20到+20），重大事件可以更大；健康遵循平衡规则
- choices：如果提供选择，必须是有意义的两难抉择，每个选项都有明确的利弊

其中，choies 是一个列表，数据示例如下：
[{"id": "1", "text": "选择打工"}, {"id": "2", "text": "继续读书"}]
...
2. 属性变化要符合选择的逻辑（例如：选择学习→智力+5~15，选择冒险→健康-3~-8且财富+15~25）
...
3. 属性影响事件：
   - 健康<30: 更多休养/康复机会，轻微病痛；健康<15: 可能出现重病并给出原因
   - 智力>80: 学业/职业上有突出表现
   - 魅力>80: 社交/恋爱机遇增多
   - 财富<10: 经济困难影响生活
   - 幸福<30: 可能出现心理问题
